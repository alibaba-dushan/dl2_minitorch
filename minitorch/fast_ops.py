    ...
    * When `out` and `in` are stride-aligned, avoid indexing
    ...
    def _map(
        ...
        if np.array_equal(out_shape, in_shape) and np.array_equal(out_strides, in_strides):
            ...
            for i in prange(len(out)):
                out_index, in_index = np.empty(MAX_DIMS, dtype=np.int32), np.empty(MAX_DIMS, dtype=np.int32)
    ...
    * When `out`, `a`, `b` are stride-aligned, avoid indexing
    ...
    def _zip(
        ...
        if np.array_equal(out_shape, a_shape) and np.array_equal(out_shape, b_shape) and np.array_equal(out_strides, a_strides) and np.array_equal(out_strides, b_strides):
            ...
            for i in prange(len(out)):
                out_index, a_index, b_index = np.empty(MAX_DIMS, dtype=np.int32), np.empty(MAX_DIMS, dtype=np.int32), np.empty(MAX_DIMS, dtype=np.int32)
    ...
    for n in prange(out_shape[0]):
        for i in range(out_shape[1]):
            for j in range(out_shape[2]):
                a_pos, b_pos = n * a_batch_stride + i * a_strides[1], n * b_batch_stride + j * b_strides[2]
                acc = 0.0
